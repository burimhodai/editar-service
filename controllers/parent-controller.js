...
module.exports = {
    ...
    createParent: async (req,res) => {
        try {
            const existsBirthNumber = await child.findOne({birthNumber:req.body.birthNumber})

            if (!existsBirthNumber) {
                return res.status(403).json({message: "The birth number is not a registered student."})
            }
    ...
    getParentbyID: async (req,res) => {
        const getParent = await parent.findById(req.params.id)
        .select("-password")
    ...
    deleteParent: async (req,res) => {
        ...
        return res.status(200).json({message:"Parent account deleted successfully"})
    }
}